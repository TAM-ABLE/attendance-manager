...
DECLARE
    ...
    v_sato_id UUID := gen_random_uuid();
...
BEGIN
    ...
    -- ============================
    -- Users (Supabase Auth)
    -- Password: "password123" (bcrypt hash with $2a$ prefix for Supabase compatibility)
    -- Note: Supabase Auth uses $2a$ prefix, not $2b$
    -- ============================

    -- Insert into auth.users (this will trigger handle_new_user to create profiles)
    -- Note: GoTrue expects string columns to be empty string '', not NULL
    ...
    -- Also insert identities for each user (required for Supabase Auth to work properly)
    -- provider_id must be the email address for email provider
    INSERT INTO auth.identities (
        ...
        v_admin_id,
        v_admin_id,
        jsonb_build_object('sub', v_admin_id::text, 'email', 'admin@example.com', 'email_verified', true),
        'email',
        'admin@example.com',
        now(),
        now(),
    ...
    INSERT INTO auth.identities (
        ...
        v_user_id,
        v_user_id,
        jsonb_build_object('sub', v_user_id::text, 'email', 'user@example.com', 'email_verified', true),
        'email',
        'user@example.com',
        now(),
        now(),
    ...
    INSERT INTO auth.identities (
        ...
        v_sato_id,
        v_sato_id,
        jsonb_build_object('sub', v_sato_id::text, 'email', 'sato@example.com', 'email_verified', true),
        'email',
        'sato@example.com',
        now(),
        now(),
